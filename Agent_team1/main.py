import sys
sys.path.insert(0,r"D:\pypy\Agents")

from AgentsHub.Supervisor import *
from AgentsHub.Crawl_news_agent import *
from langchain_openai import ChatOpenAI
from langchain_community.tools.tavily_search import TavilySearchResults
def main():
    llm = ChatOpenAI(model = "gpt-4o")
    toolsearch  = TavilySearchResults(max_results=5)
    Researcher = Agent(name="Researcher",
                    description="",
                    prompts="""You are a researcher. Your responsibility is searching and finding some images
                    and their url for Dev to code web. You have to just return the image url.
                    """,
                    tools = [toolsearch],
                    llm = llm)
    
    Dev = Agent(name="Developer",
                    description="",
                    prompts="""You are a developer that can code Front end. 
                    You just only return code. Save code if user requires
                    """,
                    tools = [save_code],
                    llm = llm)
    
    
    
    
    agents = [Dev,Researcher]
    
    super_graph = Supervise_graph(llm=llm,
                                  members = agents,
                                  member_nodes=[a.agent_node for a in agents],
                                  )
    
    return super_graph

if __name__=="__main__":
    with open("request.txt","r") as file:
        request = file.read()
    graph = main()
    print(graph.make_request(request,stream = False))